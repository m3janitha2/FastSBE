...
public:
	...
	S_VAR_LEN_DATA_NAME& AppendS_VAR_LEN_DATA_NAME(const S_VAR_LEN_DATA_VAR_DATA_TYPE* value, S_VAR_LEN_DATA_LENGTH_TYPE length) noexcept
	{
		auto* buf = buffer() + S_VAR_LEN_SNAKE_offset();
	...
	S_VAR_LEN_DATA_NAME& AppendS_VAR_LEN_DATA_NAME(std::basic_string_view<S_VAR_LEN_DATA_VAR_DATA_TYPE> value) noexcept
	{
		auto* buf = buffer() + S_VAR_LEN_SNAKE_offset();
		auto& data = *reinterpret_cast<S_VAR_LEN_DATA_NAME*>(buf);
