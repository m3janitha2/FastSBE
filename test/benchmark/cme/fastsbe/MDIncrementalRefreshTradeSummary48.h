...
class MDIncrementalRefreshTradeSummary48
{
    ...
    class NoMDEntries
    {
    	friend MDIncrementalRefreshTradeSummary48;
...
inline std::basic_ostream<CharT, Traits> &operator<<(std::basic_ostream<CharT, Traits> &os, const fastsbe::MDIncrementalRefreshTradeSummary48::NoMDEntries &group)
{
	for (auto i = 0; i < group.num_in_group(); i++)
	{
		...
		os << g.aggressor_side_name() << ": " << g.aggressor_side() << " ";
		os << g.md_update_action_name() << ": " << g.md_update_action() << " ";
		os << g.md_entry_type_name() << ": " << g.md_entry_type() << " ";
		os << g.md_trade_entry_id_name() << ": " << g.md_trade_entry_id() << " ";
	}
	return os;
}

template <class CharT, class Traits = std::char_traits<CharT>>
inline std::basic_ostream<CharT, Traits> &operator<<(std::basic_ostream<CharT, Traits> &os, const fastsbe::MDIncrementalRefreshTradeSummary48::NoOrderIDEntries &group)
{
	for (auto i = 0; i < group.num_in_group(); i++)
	{
		auto &g = group.get(i);
		os << g.order_id_name() << ": " << g.order_id() << " ";
		os << g.last_qty_name() << ": " << g.last_qty() << " ";
	}
	return os;
}

template <class CharT, class Traits = std::char_traits<CharT>>
inline std::basic_ostream<CharT, Traits> &operator<<(std::basic_ostream<CharT, Traits> &os, const fastsbe::MDIncrementalRefreshTradeSummary48 &msg)
{
