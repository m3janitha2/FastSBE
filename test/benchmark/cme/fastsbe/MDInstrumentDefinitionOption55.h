...
class MDInstrumentDefinitionOption55
{
    ...
    private:
    	PutOrCall::Value put_or_call_{};
    
    public:
    	static constexpr std::size_t put_or_call_size() noexcept
    	{
    		return sizeof(PutOrCall);
    	}
    
    	static constexpr std::size_t put_or_call_offset() noexcept
    	{ 
    		return 71; 
    	}
    
    	static constexpr std::size_t put_or_call_id() noexcept
    	{ 
    		return 201; 
    	}
    
    	static constexpr const char* put_or_call_name() noexcept
    	{ 
    		return "PutOrCall"; 
    	}
    
    	static constexpr PutOrCall::Value put_or_call_null_value() noexcept
    	{
    		return PutOrCall::Value::nullValue;
    	}
    
    	constexpr PutOrCall::Value put_or_call() const noexcept
    	{ 
    		return put_or_call_;
    	}
    	
    	constexpr auto& set_put_or_call(PutOrCall::Value value) noexcept
    	{
    		put_or_call_ = value;
    		return *this;
    	}
    ...
    private:
    	Pricenull9 strike_price_{};
    
    public:
    	static constexpr std::size_t strike_price_size() noexcept
    	{
    		return sizeof(Pricenull9);
    	}
    
    	static constexpr std::size_t strike_price_offset() noexcept
    	{
    		return 80;
    	}
    
    	static constexpr std::size_t strike_price_id() noexcept
    	{
    		return 202;
    	}
    
    	static constexpr const char *strike_price_name() noexcept
    	{
    		return "StrikePrice";
    	}
    
    	constexpr const Pricenull9 &strike_price() const noexcept
    	{
    		return strike_price_;
    	}
    
    	constexpr Pricenull9 &strike_price() noexcept
    	{
    		return strike_price_;
    	}
    	constexpr auto &set_strike_price(Pricenull9 &value) noexcept
    	{
    		strike_price_ = value;
    		return *this;
    	}
    ...
    private:
    	char strike_currency_[3]{};
    
    public:
    	static constexpr std::size_t strike_currency_size() noexcept
    	{
    		return 3;
    	}
    
    	static constexpr std::size_t strike_currency_offset() noexcept
    	{
    		return 88;
    	}
    
    	static constexpr std::size_t strike_currency_id() noexcept
    	{
    		return 947;
    	}
    
    	static constexpr const char *strike_currency_name() noexcept
    	{
    		return "StrikeCurrency";
    	}
    
    	constexpr char *strike_currency() noexcept
    	{
    		return strike_currency_;
    	}
    
    	constexpr const char *strike_currency() const noexcept
    	{
    		return strike_currency_;
    	}
    
    	constexpr std::string_view strike_currency_view() noexcept
    	{
    		return std::string_view(strike_currency_, 3);
    	}
    
    	constexpr const std::string_view strike_currency_view() const noexcept
    	{
    		return std::string_view(strike_currency_, 3);
    	}
    
    	constexpr const std::string strike_currency_string() const noexcept
    	{
    		auto length = 0ULL;
    		for (; length < 3 && *(strike_currency_ + length) != '\0'; ++length);
    		return std::string(strike_currency_, length);
    	}
    
    	constexpr auto &set_strike_currency(const char *value) noexcept
    	{
    	#if defined(__GNUG__)
    	#pragma GCC diagnostic push
    	#pragma GCC diagnostic ignored "-Wstringop-overflow"
    ...
    private:
    	Pricenull9 min_cab_price_{};
    
    public:
    	static constexpr std::size_t min_cab_price_size() noexcept
    	{
    		return sizeof(Pricenull9);
    	}
    
    	static constexpr std::size_t min_cab_price_offset() noexcept
    	{
    		return 94;
    	}
    
    	static constexpr std::size_t min_cab_price_id() noexcept
    	{
    		return 9850;
    	}
    
    	static constexpr const char *min_cab_price_name() noexcept
    	{
    		return "MinCabPrice";
    	}
    
    	constexpr const Pricenull9 &min_cab_price() const noexcept
    	{
    		return min_cab_price_;
    	}
    
    	constexpr Pricenull9 &min_cab_price() noexcept
    	{
    		return min_cab_price_;
    	}
    	constexpr auto &set_min_cab_price(Pricenull9 &value) noexcept
    	{
    		min_cab_price_ = value;
    		return *this;
    	}
    ...
    class NoEvents
    {
    	template <std::size_t> friend class MDInstrumentDefinitionOption55;
...
inline std::basic_ostream<CharT, Traits> &operator<<(std::basic_ostream<CharT, Traits> &os, const fastsbe::MDInstrumentDefinitionOption55<N> &msg)
{
	os << "{";
	bool comma = false;
