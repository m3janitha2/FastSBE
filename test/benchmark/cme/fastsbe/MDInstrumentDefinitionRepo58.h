...
class MDInstrumentDefinitionRepo58
{
    ...
    public:
    	...
    	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
    	auto &set_party_role_clearing_org(const char *value, std::size_t size) noexcept
    	{
    		const auto length = size < party_role_clearing_org_size() ? size : party_role_clearing_org_size();
    		std::memcpy(party_role_clearing_org_, value, length);
    		std::memset(party_role_clearing_org_ + length, 0, party_role_clearing_org_size() - length);
    		return *this;
    	}
    ...
    public:
    	...
    	constexpr const std::string termination_type_string() const noexcept
    	{
    		auto length = 0ULL;
    	...
    	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
    	auto &set_termination_type(const char *value, std::size_t size) noexcept
    	{
    		const auto length = size < termination_type_size() ? size : termination_type_size();
    		std::memcpy(termination_type_, value, length);
    		std::memset(termination_type_ + length, 0, termination_type_size() - length);
    		return *this;
    	}
    ...
    public:
    	...
    	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
    	auto &set_price_quote_method(const char *value, std::size_t size) noexcept
    	{
    		const auto length = size < price_quote_method_size() ? size : price_quote_method_size();
    		std::memcpy(price_quote_method_, value, length);
    		std::memset(price_quote_method_ + length, 0, price_quote_method_size() - length);
    		return *this;
    	}
    ...
    public:
    	...
    	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
    	auto &set_risk_set(const char *value, std::size_t size) noexcept
    	{
    		const auto length = size < risk_set_size() ? size : risk_set_size();
    		std::memcpy(risk_set_, value, length);
    		std::memset(risk_set_ + length, 0, risk_set_size() - length);
    		return *this;
    	}
    ...
    public:
    	...
    	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
    	auto &set_market_set(const char *value, std::size_t size) noexcept
    	{
    		const auto length = size < market_set_size() ? size : market_set_size();
    		std::memcpy(market_set_, value, length);
    		std::memset(market_set_ + length, 0, market_set_size() - length);
    		return *this;
    	}
    ...
    public:
    	...
    	constexpr const std::string term_code_string() const noexcept
    	{
    		auto length = 0ULL;
    	...
    	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
    	auto &set_term_code(const char *value, std::size_t size) noexcept
    	{
    		const auto length = size < term_code_size() ? size : term_code_size();
    		std::memcpy(term_code_, value, length);
    		std::memset(term_code_ + length, 0, term_code_size() - length);
    		return *this;
    	}
    ...
    class NoUnderlyings
    {
        ...
        class Entry
        {
            ...
            public:
            	...
            	constexpr const std::string underlying_symbol_string() const noexcept
            	{
            		auto length = 0ULL;
            ...
            public:
            	...
            	constexpr const std::string underlying_security_alt_id_string() const noexcept
            	{
            		auto length = 0ULL;
            ...
            public:
            	...
            	constexpr const std::string underlying_financial_instrument_full_name_string() const noexcept
            	{
            		auto length = 0ULL;
            	...
            	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
            	auto &set_underlying_financial_instrument_full_name(const char *value, std::size_t size) noexcept
            	{
            		const auto length = size < underlying_financial_instrument_full_name_size() ? size : underlying_financial_instrument_full_name_size();
            		std::memcpy(underlying_financial_instrument_full_name_, value, length);
            		std::memset(underlying_financial_instrument_full_name_ + length, 0, underlying_financial_instrument_full_name_size() - length);
            		return *this;
            	}
            ...
            public:
            	...
            	constexpr const std::string underlying_security_type_string() const noexcept
            	{
            		auto length = 0ULL;
            	...
            	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
            	auto &set_underlying_security_type(const char *value, std::size_t size) noexcept
            	{
            		const auto length = size < underlying_security_type_size() ? size : underlying_security_type_size();
            		std::memcpy(underlying_security_type_, value, length);
            		std::memset(underlying_security_type_ + length, 0, underlying_security_type_size() - length);
            		return *this;
            	}
            ...
            public:
            	...
            	constexpr const std::string underlying_country_of_issue_string() const noexcept
            	{
            		auto length = 0ULL;
            	...
            	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
            	auto &set_underlying_country_of_issue(const char *value, std::size_t size) noexcept
            	{
            		const auto length = size < underlying_country_of_issue_size() ? size : underlying_country_of_issue_size();
            		std::memcpy(underlying_country_of_issue_, value, length);
            		std::memset(underlying_country_of_issue_ + length, 0, underlying_country_of_issue_size() - length);
            		return *this;
            	}
            ...
            public:
            	...
            	constexpr const std::string underlying_issuer_string() const noexcept
            	{
            		auto length = 0ULL;
            	...
            	// Safe: copy size bytes (capped at the field width) and NUL-pad the remainder.
            	auto &set_underlying_issuer(const char *value, std::size_t size) noexcept
            	{
            		const auto length = size < underlying_issuer_size() ? size : underlying_issuer_size();
            		std::memcpy(underlying_issuer_, value, length);
            		std::memset(underlying_issuer_ + length, 0, underlying_issuer_size() - length);
            		return *this;
            	}
    ...
    public:
    	constexpr NoUnderlyings() = default;
    	NoUnderlyings(std::uint8_t count) noexcept
    ...
    class NoRelatedInstruments
    {
        ...
        class Entry
        {
            ...
            public:
            	...
            	constexpr const std::string related_symbol_string() const noexcept
            	{
            		auto length = 0ULL;
    ...
    public:
    	constexpr NoRelatedInstruments() = default;
    	NoRelatedInstruments(std::uint8_t count) noexcept
    ...
    public:
    	constexpr NoBrokenDates() = default;
    	NoBrokenDates(std::uint8_t count) noexcept
...
inline std::basic_ostream<CharT, Traits> &operator<<(std::basic_ostream<CharT, Traits> &os, const fastsbe::MDInstrumentDefinitionRepo58<N> &msg)
{
	...
	if(comma) { os << ", "; } os << "\"StartDate\": " << msg.start_date(); comma = true;
	if(comma) { os << ", "; } os << "\"EndDate\": " << msg.end_date(); comma = true;
	if(comma) { os << ", "; } os << "\"TerminationType\": " << "\"" << msg.termination_type() << "\""; comma = true;
	if(comma) { os << ", "; } os << "\"SecuritySubType\": " << "\"" << msg.security_sub_type() << "\""; comma = true;
	if(comma) { os << ", "; } os << "\"MoneyOrPar\": " << "\"" << msg.money_or_par() << "\""; comma = true;
	if(comma) { os << ", "; } os << "\"MaxNoOfSubstitutions\": " << +msg.max_no_of_substitutions(); comma = true;
	...
	if(comma) { os << ", "; } os << "\"TermCode\": " << "\"" << msg.term_code() << "\""; comma = true;
	if(comma) { os << ", "; } os << "\"BrokenDateTermType\": " << +msg.broken_date_term_type(); comma = true;
	...
	if(comma) { os << ", "; } os << "\"NoBrokenDates\": " << msg.no_broken_dates(); comma = true;
	os << "}";
	return os;
}
}
